...
void loadTable() {
  imageTable = loadTable("data/rgb_map_clustered.csv", "header");
  PVector position;
  Flock flock;
  for (TableRow row : imageTable.rows()) {
    ...
    flock.addBoid(
      new Boid(x, y, new PVector(r, g, b), imageSize * 2, clusterClusters.get(clusterCluster).position, forceField)
    );
  }
